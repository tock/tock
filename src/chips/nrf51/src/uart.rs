...
impl UART {
    ...
    fn configure(&mut self, baud_rate: u32) {
		...
		unsafe{intrinsics::volatile_store(&mut regs.enable, 0b100);}
    ...
    pub fn enable_rx_interrupts(&self) {
        let regs : &mut Registers = unsafe { mem::transmute(self.regs) };
        unsafe {intrinsics::volatile_store(&mut regs.intenset, 1 << 3 as u32);}
    }

    pub fn enable_tx_interrupts(&self) {
        let regs : &mut Registers = unsafe { mem::transmute(self.regs) };
        unsafe {intrinsics::volatile_store(&mut regs.intenset, 1 << 7 as u32);}
    }

    pub fn disable_rx_interrupts(&self) {
        let regs : &mut Registers = unsafe { mem::transmute(self.regs) };
        unsafe {intrinsics::volatile_store(&mut regs.intenclr, 1 << 3 as u32);}
    }

    pub fn disable_tx_interrupts(&self) {
        let regs : &mut Registers = unsafe { mem::transmute(self.regs) };
        unsafe {intrinsics::volatile_store(&mut regs.intenclr, 1 << 7 as u32);}
    }
...
impl uart::UART for UART {
    ...
    fn send_byte(&self, byte: u8) {
        ...
        while !self.tx_ready() {}
        unsafe{intrinsics::volatile_store(&mut regs.stoptx, 1 as u32);}
    }
