...
int main(void)
{
  ...
  int err = aes128_configure_key(key, sizeof(key));
  if (err < 0) printf("set key error %d\r\n", err);

  err = aes128_encrypt_ctr(data, sizeof(data), ctr, sizeof(ctr));
  ...
  err = aes128_decrypt_ctr(data, sizeof(data), ctr, sizeof(ctr));
