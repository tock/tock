...
impl<'a, A: AES128<'a> + AES128Ctr> EncryptionOracleDriver<'a, A> {
    ...
    /// Return a `ProcessId` which has `request_pending` set.
            ...
            .enter(processid, |grant, kernel_data| {
                ...
                if grant.offset == 0 {
                    ...
                    self.aes.set_mode_aes128ctr(
                        false, // encrypting = false -> decrypt!
                    )?;
                ...
                source_processbuffer.enter(|source_processbuffer| {
                        ...
                        AES128::crypt(self.aes, Some(source_buffer), dest_buffer, 0, crypt_len)
                    {
                        ...
                        // and replace the buffers.
                        //
                        // TODO: inform the client of the error instead of
                        // printing this message.
                        kernel::debug!("AES128 crypt error: {:?}", e);
                        self.aes.disable();
    ...
    // This should be implemented as part of the tutorial:
    fn run_next_pending(&self) {
        ...
        while let Some(processid) = self.next_pending() {
            if let Err(_e) = self.run(processid) {
                // Errored, remove pending indication:
                self.process_grants.enter(processid, |grant, _| {
