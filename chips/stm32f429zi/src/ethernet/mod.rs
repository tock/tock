...
//! # Usage
//!
//! The Ethernet peripheral requires a reference clock. Check the board's documentation to see if
//! it is generated by the PHY or not. In the case it is not, then it can be generated by using
//! one of the micro-controller output clocks (MCO). For this uncomment the MCO part inside the
//! **setup_ethernet_gpios()** function and connect the corresponding pins.
//!
//! To setup the Ethernet peripheral, add the following lines before entering the kernel main loop:
//!
//! ```rust,ignore
