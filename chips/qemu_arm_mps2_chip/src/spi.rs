...
//!   therefore always enables `CR1.LBM` (loopback) in [`Spi::init`] — the
//!   only way to get a genuine, deterministic transfer under this model.
//! - **No functional chip select.** Real `SpiMaster` implementations
//!   elsewhere toggle a GPIO pin for chip select, but GPIO is a QEMU stub
//!   on this machine (see `led.rs`'s module docs). [`ChipSelect`] is a
//!   zero-sized placeholder that satisfies the trait without touching any
//!   hardware.
//!
