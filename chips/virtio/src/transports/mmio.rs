...
impl VirtIOMMIODevice<virtio_mmio_device_registers::Real<Mmio32>> {
    /// Safety: This must be called on a riscv32 system with VirtIO.
    pub unsafe fn new_all() -> [Self; 8] {
        ADDRESSES.map(|address| {
            let mmio = Mmio32::from_addr(address);
            VirtIOMMIODevice {
                // Safety:
                // 1. The caller has guaranteed that this system has VirtIO, and
                //    VirtIO on riscv32 is always MMIO at the addresses in
                //    `ADDRESSES`, so `mmio` is a correct address.
                // 2. virtio_mmio_device_registers was defined correctly.
                // 3. There's no way to cause a data race through VirtIO.
...
impl VirtIOMMIODevice<virtio_mmio_device_registers::Real<Mmio64>> {
    /// Safety: This must be called on a riscv64 system with VirtIO.
    pub unsafe fn new_all() -> [Self; 8] {
        ADDRESSES.map(|address| {
            ...
            VirtIOMMIODevice {
                // Safety:
                // 1. The caller has guaranteed that this system has VirtIO, and
                //    VirtIO on riscv64 is always MMIO at the addresses in
                //    `ADDRESSES`, so `mmio` is a correct address.
                // 2. virtio_mmio_device_registers was defined correctly.
                // 3. There's no way to cause a data race through VirtIO.
                regs: unsafe { virtio_mmio_device_registers::Real::new(mmio) },
