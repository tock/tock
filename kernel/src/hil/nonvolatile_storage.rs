...
pub trait NonvolatileStorage<'client, 'buffer: 'client> {
    fn set_client(&self, client: &'client NonvolatileStorageClient<'buffer>);
    ...
    fn read(&self, buffer: &'buffer mut [u8], address: usize, length: usize) -> ReturnCode;
    ...
    /// must be in the address space of the physical storage.
    fn write(&self, buffer: &'buffer mut [u8], address: usize, length: usize) -> ReturnCode;
}

/// Client interface for nonvolatile storage.
pub trait NonvolatileStorageClient<'buffer> {
    ...
    /// were actually read.
    fn read_done(&self, buffer: &'buffer mut [u8], length: usize);

    /// `write_done` is called when the implementor is finished writing from the
    /// buffer. The callback returns the buffer and the number of bytes that
    /// were actually written.
    fn write_done(&self, buffer: &'buffer mut [u8], length: usize);
}
