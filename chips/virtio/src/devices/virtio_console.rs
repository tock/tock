...
//! Driver for the VirtIO console device (device type 3), exposed by QEMU
//! (and other hypervisors) as `virtio-serial-device` / `virtio-serial-pci` /
//! etc.
//!
//! This driver implements [`hil::uart::Configure`], [`hil::uart::Transmit`],
//! and [`hil::uart::Receive`], letting a virtio console be used anywhere
//! Tock expects a UART.
//!
//! Only the basic (non-multiport) console mode is supported: this driver
//! does not negotiate `VIRTIO_CONSOLE_F_MULTIPORT`, so the device exposes
//! exactly two virtqueues -- a receive queue (0) and a transmit queue (1) --
//! with no per-buffer header, unlike e.g. `virtio-net`.
//!
//! Because the device delivers data from the host in host-chosen chunks
//! (there is no way for the guest to request "exactly N bytes"), this
//! driver always keeps a single one-byte buffer posted to the receive
//! queue while a [`hil::uart::Receive::receive_buffer`] call is
//! outstanding, and copies each incoming byte into the client's buffer.
//! This avoids ever having to buffer or discard partial chunks, at the
//! cost of one virtqueue round-trip per received byte -- acceptable for a
//! console/debug UART, but not a high-throughput link.
