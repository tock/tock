...
use kernel::utilities::streaming_process_slice::{
    StreamingProcessSlice, StreamingProcessSliceMeta,
};
    ...
    for CanCapsule<'a, Can>
{
    ...
    fn message_received(
        ...
        match status {
            Ok(()) => {
                let res: Result<StreamingProcessSliceMeta, ErrorCode> =
                ...
                match res {
                    ...
                    Ok(new_meta) => self.schedule_callback(
                        up_calls::UPCALL_MESSAGE_RECEIVED,
                        (
                            0,
                            new_meta.offset as usize,
                            match id {
