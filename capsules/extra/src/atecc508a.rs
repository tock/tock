    ...
    fn command_complete(&self, buffer: &'static mut [u8], status: Result<(), i2c::Error>) {
        match self.op.get() {
            ...
            Operation::ReadConfigZeroCommand => {
                self.op.set(Operation::ReadConfigZeroResult);

                self.i2c
                    .read(buffer, self.op_len.get() + RESPONSE_COUNT_SIZE + CRC_SIZE)
                    .unwrap();
            }
            Operation::ReadConfigZeroResult => {
            ...
            Operation::GenerateEntropyCommand => {
...
impl Iterator for Atecc508aRngIter<'_, '_> {
    ...
    fn next(&mut self) -> Option<u32> {
        self.0.entropy_buffer.take().map_or(None, |entropy_buffer| {
            let entropy_bytes = <[u8; 4]>::try_from(&entropy_buffer[0..4]).unwrap();
            let entropy: u32 = u32::from_le_bytes(entropy_bytes);
...
impl<'a> entropy::Entropy32<'a> for Atecc508a<'a> {
    ...
    fn get(&self) -> Result<(), ErrorCode> {
        self.op.set(Operation::GenerateEntropyCommand);
