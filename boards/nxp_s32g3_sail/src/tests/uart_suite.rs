...
//! Sequential UART test suite for LinFlexD on LF1.
//!
//! Runs four test phases in sequence, advancing via callbacks:
//!   Phase 1: RX buffer → wait for 4 bytes from human (press 4 keys on LF1)
...
impl UartTestSuite {
    const fn new() -> Self {
        Self {
            ...
            phase: Cell::new(Phase::RxSuccess),
    ...
    fn run_rx_success(&self) {
        debug!(
            "[Phase 1] RX buffer success — press {} keys on LF1...",
            RX_LEN
        );
        self.uart.map(|uart| {
        ...
        match self.phase.get() {
            ...
            Phase::TxAbort => {
                if rval == Err(ErrorCode::CANCEL) {
                    ...
                    // Race: HW completed before abort took effect.
        ...
        match self.phase.get() {
            Phase::RxSuccess => {
                if rval == Ok(()) && rx_len == RX_LEN && error == uart::Error::None {
                    ...
                    // Echo what was received for visual confirmation.
                    self.rx_buf.map(|buf| {
                        debug!(
...
pub unsafe fn run(lf1: &'static LinFlexD<'static>) {
    ...
    debug!("========================================");
    debug!("[UART SUITE] Starting LinFlexD test suite on LF1 @ 921600");
    debug!("[UART SUITE] Phase 1 requires human input: press 4 keys on LF1");
    debug!("========================================");
