...
struct QemuRv32VirtPlatform {
    ...
    scheduler_timer: &'static VirtualSchedulerTimer<
        VirtualMuxAlarm<'static, qemu_rv32_virt_chip::chip::QemuRv32VirtClint<'static>>,
    >,
    virtio_rng: Option<&'static core_capsules::rng::RngDriver<'static>>,
}
...
pub unsafe fn main() {
    ...
    let virtio_rng_driver: Option<&'static core_capsules::rng::RngDriver<'static>> =
        if let Some(rng_idx) = virtio_rng_idx {
            ...
            let rng_driver: &'static mut core_capsules::rng::RngDriver = static_init!(
                core_capsules::rng::RngDriver,
                core_capsules::rng::RngDriver::new(
                    rng,
                    board_kernel
                        .create_grant(core_capsules::rng::DRIVER_NUM, &memory_allocation_cap),
                ),
            );
            rng.set_client(rng_driver);

            Some(rng_driver as &'static core_capsules::rng::RngDriver)
        } else {
