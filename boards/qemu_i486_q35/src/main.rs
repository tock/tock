...
use virtio::transports::pci::VirtIOPCIDevice;
...
fn init_virtio_dev(dev: Device, dev_type: VirtIODeviceType) -> Option<(u8, VirtIOPCIDevice)> {
    use pci::cap::Cap;
...
unsafe extern "cdecl" fn main() {
    ...
    for dev in pci::iter() {
        use virtio::devices::VirtIODeviceType;
        use virtio::transports::pci::{DEVICE_ID_BASE, VENDOR_ID};
