 ...
 * supervisor. There is one utest_t structure per test runner (one test runner
 * per process).
 */
typedef struct utest_t utest_t;
struct utest_t {
 ...
 * utest_t struct. Only needed for optimizing the append operation.
 */
...
/**
 * Append a new utest_t to the linked list.
 */
static void list_append(linked_list_t *list, utest_t *test) {
    if (list->tail) {
        list->tail->next = test;
    } else {
        list->head = test;
...
/**
 * Remove a utest_t from the front of the linked list.
 */
...
/**
 * Returns true if the linked list contains the given utest_t.
 */
static bool list_contains(linked_list_t *list, utest_t *test) {
    utest_t *curr = list->head;
    while (curr) {
...
/**
 * Test runner's shared buffer which holds the per-test-runner utest_t state.
...
void utest_runner(utest_fun *tests, uint32_t test_count,
    ...
    utest_t *test = (utest_t *)(&test_buf[0]);
 ...
 */
static void print_test_result(utest_t *test) {
 ...
 */
static void print_test_summary(utest_t *test) {
 ...
 * supervisor (service) side. See utest_runner for details about the test runner
 * (client) side.
 */
static void utest_service_cb(int pid,
...
void utest_service(void) {
    ...
    ipc_register_svc(utest_service_cb, &pending_pids);
}
