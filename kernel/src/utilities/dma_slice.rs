...
// element. However, this is not yet available on stable. We provide the
// following stable method instead.
//
// TODO: remove once `slice_ptr_get` is stable.
#[inline(always)]
fn slice_ptr_as_mut_ptr<T>(slice_ptr: *const [T]) -> *mut T {
    slice_ptr as *mut T
}
...
// TODO: remove once `slice_ptr_get` is stable.
#[inline(always)]
fn mut_slice_ptr_as_mut_ptr<T>(slice_ptr: *mut [T]) -> *mut T {
    slice_ptr as *mut T
}
...
impl<'a, T> DmaSlice<'a, T> {
    ...
    pub fn from_slice_ref(slice: &[T], fence: impl DmaFence) -> DmaSlice<'_, T> {
        ...
        // function returns.
        //
        // Clippy says we should be using `as_mut_ptr`, but that method doesn't
        // exist. The cast doesn't matter here, `DmaFence::release` will not
        // actually dereference the memory.
        #[allow(clippy::as_ptr_cast_mut)]
        fence.release::<T>(slice.as_ptr() as *mut T, slice.len());
...
impl<'a, T> DmaSliceMut<'a, T> {
    ...
    pub unsafe fn from_mut_slice_ref(slice: &mut [T], fence: impl DmaFence) -> DmaSliceMut<'_, T> {
        ...
        // function returns:
        fence.release::<T>(
            slice_ptr_as_mut_ptr(dma_slice_mut.slice_ptr.as_ptr()),
            dma_slice_mut.slice_ptr.len(),
        );
    ...
    pub fn as_mut_ptr(&self) -> *mut T {
        mut_slice_ptr_as_mut_ptr(self.slice_ptr.as_ptr())
    }
    ...
    pub unsafe fn restore_mut_slice_ref(mut self, fence: impl DmaFence) -> &'a mut [T] {
        // Ensure that any reads from Rust to the buffer described by `ptr` and
        // `len` _after_ this function returns reflect all writes made by DMA
        // operations finished _before_ this function ran:
        fence.acquire::<T>(
            mut_slice_ptr_as_mut_ptr(self.slice_ptr.as_ptr()),
            self.slice_ptr.len(),
        );
...
impl<'a, T> DmaSubSliceMut<'a, T> {
    ...
    pub unsafe fn from_sub_slice_mut(
        ...
        // operations initiated by an MMIO read or write operation after this
        // function returns:
        fence.release::<T>(dma_sub_slice_mut.as_mut_ptr(), dma_sub_slice_mut.len());
    ...
    /// wrapped [`SubSliceMut`].
    pub fn as_mut_ptr(&self) -> *mut T {
        slice_ptr_as_mut_ptr(self.internal_slice_ptr.as_ptr()).wrapping_add(
    ...
    pub unsafe fn restore_sub_slice_mut(mut self, fence: impl DmaFence) -> SubSliceMut<'a, T> {
        ...
        // `len` _after_ this function returns reflect all writes made by DMA
        // operations finished _before_ this function ran:
        fence.acquire::<T>(self.as_mut_ptr(), self.len());
