...
impl<'a, I: I2CDevice> I2CClient for I2cNmea<'a, I> {
    fn command_complete(&self, buffer: &'static mut [u8], status: Result<(), i2c::Error>) {
        let nmea_buf = self.sentence_buffer.take().unwrap();
        let i2c_buf_len = buffer.len();
        ...
        if let Some(location) = string.find('$') {
            ...
            nmea_buf[nmea_offset..(nmea_offset + location)].copy_from_slice(&buffer[0..location]);
            ...
            if sentence.starts_with('$') {
                ...
                nmea_buf[nmea_offset..(nmea_offset + size)].copy_from_slice(&buffer[location..]);
                nmea_offset += size;
            }
        } else {
