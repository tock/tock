...
pub unsafe fn run_multi_alarm(mux: &'static MuxAlarm<'static, Ast<'static>>) {
    ...
    let tests: [&'static TestRandomAlarm<'static, VirtualMuxAlarm<'static, Ast<'static>>>; 3] = static_init_multi_alarm_test(mux);
    tests[0].run();
    tests[1].run();
    tests[2].run();
}

unsafe fn static_init_multi_alarm_test(mux: &'static MuxAlarm<'static, Ast<'static>>) -> [&'static TestRandomAlarm<'static, VirtualMuxAlarm<'static, Ast<'static>>>; 3] {
