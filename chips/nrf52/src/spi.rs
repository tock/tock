...
use kernel::hil::spi::{ChipSelectActivePolarity, ChipSelectPin};
...
pub struct SPIM<'a> {
    ...
    chip_select: OptionalCell<ChipSelectPin<&'a crate::gpio::GPIOPin<'a>>>,
    busy: Cell<bool>,
...
impl<'a> hil::spi::SpiMaster<'a> for SPIM<'a> {
    ...
    fn specify_chip_select(
        ...
        cs.make_output();
        self.chip_select.set(ChipSelectPin::new(cs, polarity));
        Ok(())
    }
