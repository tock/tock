...
//! ----------------
//!
//! This driver exposes the HTS221's temperature and humidity functionality via
//! the [TemperatureDriver] and [HumidityDriver] HIL interfaces. The driver
//! _does not_ attempt to support multiple concurrent requests for temperature
//! or multiple concurrent requests for humidity, but _does_ support a
//! concurrent request each for temperature and humidity. It is not the role of
//! this driver to provide virtualization to multiple clients, but it does
//! provide virtualization to allow it to be used from both a temperature and
//! humidity driver.
//!
//! Specifically, the implementation _always_ reads both temperature and
//! humidity (the chip always provides both anyway). If the driver receives a
//! request for either temperature or humidity while a request for the other is
//! outstanding, both will be returned to their respective clients when the I2C
//! transaction is completed, rather than performing two separate transactions.
//!
//! Polling for data readiness
//! --------------------------
//!
//! The HTS221 has a data-ready line that can provide an interrupt when
//! temperature and humidity data is ready. However, the primary board (the Nano
//! 33 BLE Sense) this driver was developed for does not connect that line to
//! the MCU and, typically, data is ready within a couple read/write I2C
//! transactions. So, the driver **polls** the status register instead. This is
//! probably not optimal from an energy perspective, so should a use case for an
//! interrupt driven interface arise, some brave soul should modify the driver
//! to support both.
//!
//! Limitations
//! -----------
//!
//! The driver uses floating point math to adjust readings based on the
//! calibration registers. This is accurate and matches the chip's datasheet's
//! recommendation, but could increase code size significantly in platforms that
    ...
    fn command_complete(&self, buffer: &'static mut [u8], status: Result<(), i2c::Error>) {
        ...
        match self.state.get() {
            State::Calibrating => {
                ...
                let t0deg_c = ((buffer[2] as i16) | (((buffer[5] & 0b11) as i16) << 8)) as f32;
                let t1deg_c = ((buffer[3] as i16) | (((buffer[5] & 0b1100) as i16) << 6)) as f32;
