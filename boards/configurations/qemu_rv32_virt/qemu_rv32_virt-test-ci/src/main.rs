...
pub unsafe fn main() {
    ...
    // We define an additional ePMP region to cover the flash. Writing the
    // flash requires writing to the actual flash memory addresses.
    let epmp_flash_region = Some(rv32i::pmp::kernel_protection_mml_epmp::MiscellaneousRegion(
    ...
    let (board_kernel, base_platform, chip, peripherals) =
        qemu_rv32_virt_lib::start(epmp_flash_region);
