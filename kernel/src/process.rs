...
pub trait Process {
    ...
    ///
    /// This function returns `Ok(())` if the `Task` was successfully enqueued.
    /// If the process is no longer alive, `Err(ErrorCode::NODEVICE)` is
    /// returned. If the task could not be enqueued because there is
    /// insufficient space in the internal task queue, `Err(ErrorCode::NOMEM)`
    /// is returned. Other return values must be treated as kernel-internal
    /// errors.
    ...
    /// `Unchecked` state. The `credentials` argument is `None` if all
    /// credentials are Pass; if a credential is Accept, it is passed in
    /// `credentials`.  The `short_app_id` argument is the short ID generated by
    /// the Identifier Policy. Transitioning a process to `CredentialsApproved`
    /// has security implications because it means this application binary is
    /// permitted to run on the system. This method therefore requires a
    /// Capability.
...
/// decide if the process can run given other processes in the system.  If it
/// can run, the kernel pushes its initial stack frame and transitions it to the
