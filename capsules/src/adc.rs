    ...
    fn sample_buffer(&self, channel: usize, frequency: u32) -> ReturnCode {
        ...
        let res = self.adc_buf1.take().map_or(ReturnCode::EBUSY, |buf1| {
            self.adc_buf2.take().map_or(ReturnCode::EBUSY, move |buf2| {
                ...
                let (rc, retbuf1, retbuf2) = self.adc.sample_highspeed(
                    chan,
                    frequency,
                    buf1,
                    len1,
            ...
            self.app.map(|state| {
                    ...
                    if self.samples_outstanding.get() == 0 {
                        ...
                        if self.mode.get() == AdcMode::ContinuousBuffer {
                            ...
                            self.samples_remaining.set(
                                samples_needed -
                                    self.next_samples_outstanding
                                        .get(),
                            );
                            self.samples_outstanding.set(
                                self.next_samples_outstanding.get(),
                            );
                ...
                app_buf.map(move |app_buf| {
                    ...
                    self.replace_buffer(buf).map(|adc_buf| {
                        ...
                        // copied
                        self.app_buf_offset.set(
                            self.app_buf_offset.get() + length * 2,
                        );
                    });
...
impl<A: hil::adc::Adc + hil::adc::AdcHighSpeed> Driver for Adc<'a, A> {
    ...
    fn allow(
        ...
        match allow_num {
            ...
            0 => {
                // set first buffer
                self.app.map(|state| { state.app_buf1 = slice; });
            ...
            1 => {
                // set second buffer
                self.app.map(|state| { state.app_buf2 = slice; });
