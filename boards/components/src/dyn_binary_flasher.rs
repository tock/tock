...
//!
//! This provides one component, BinaryFlasherComponent, which provides
//! a system call interface to DynamicBinaryFlasher.
//!
//!```rust, ignore
//! # use kernel::static_init;
//!
//! let dynamic_binary_flasher = components::dyn_binary_flasher::BinaryFlasherComponent::new(
...
use kernel::dynamic_binary_flashing::DynamicBinaryFlasher;
use kernel::hil;
use kernel::process;
...
macro_rules! binary_flasher_component_static {
    ($F:ty, $C:ty $(,)?) => {{
        ...
        let pl =
            kernel::static_buf!(kernel::dynamic_binary_flashing::DynamicBinaryFlasher<'static>);
        let buffer = kernel::static_buf!([u8; kernel::dynamic_binary_flashing::BUF_LEN]);

        (page, ntp, pl, buffer)
    };};
}

pub struct BinaryFlasherComponent<
...
impl<
        F: 'static
            + hil::flash::Flash
            + hil::flash::HasClient<'static, NonvolatileToPages<'static, F>>,
    > Component for BinaryFlasherComponent<F>
{
    type StaticInput = (
        &'static mut MaybeUninit<<F as hil::flash::Flash>::Page>,
        &'static mut MaybeUninit<NonvolatileToPages<'static, F>>,
        &'static mut MaybeUninit<DynamicBinaryFlasher<'static>>,
        &'static mut MaybeUninit<[u8; kernel::dynamic_binary_flashing::BUF_LEN]>,
    );
    type Output = &'static DynamicBinaryFlasher<'static>;

    fn finalize(self, static_buffer: Self::StaticInput) -> Self::Output {
        let buffer = static_buffer
            .3
            .write([0; kernel::dynamic_binary_flashing::BUF_LEN]);
        ...
        let dynamic_binary_flasher = static_buffer.2.write(DynamicBinaryFlasher::new(
            nv_to_page,
            self.loader_driver,
            buffer,
        ));
