...
pub trait FrameBuffer<'a> {
    /// Returns a GraphicsMode struct with the current resolution (in pixels) and
    /// the pixel format of the display. This function is synchronous as the
    /// driver should know this value without requesting it from the screen.
    ///
    ...
    /// the display. This means that clients can only write entire lines to the framebuffer
    ...
    /// and after the actual framebuffer data. Without this padding, the display
...
pub trait FrameBufferSetup<'a>: FrameBuffer<'a> {
    /// Sets the graphics mode of the display. Returns ENOSUPPORT if the resolution is
    ...
    /// Returns the number of the graphic modes supported. Should return at least
    /// one (the current resolution). This function is synchronous as the driver
    /// should know this value without requesting it from the screen (most
    /// screens do not support such a request, resolutions and pixel formats
    /// are described in the data sheet).
    ///
    ...
    /// Can be called with an index from 0 .. count-1 and will return a
    /// GraphicsMode struct with the current resolution and pixel format.
    ///
    /// Note that width and height may change due to rotation
    ///
