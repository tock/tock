//! `udp_dummy.rs`: Test Usage of the udp.rs capsule. 
//! This file primarily reuses code from the lowpan_frag_dummy file, modified to
//! remove code testing different types of 6lowpan fragmentation, instead just
//! using INLINE for traffic flow, source address compression, and destination 
//! address compression
//! 
//! For this test to work, the following code must be added to 
...
//! // Imix initialization
//! ...
//! udp_test.start(); // Assumes flashing the Imix that is transmitting the first msg
...
use capsules::net::udp::{send_udp_packet, udp_ipv6_prepare_packet, TF, SAC, DAC, UDPHeader};
...
pub const SRC_ADDR: IPAddr = IPAddr([0xfe, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x0c, 0x26, 0x69,
                                     0x15, 0xb5, 0xbf, 0x0a]);
pub const DST_ADDR: IPAddr = IPAddr([0xff, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x02]);
pub const SRC_MAC_ADDR: MacAddress = MacAddress::Long([0xfe, 0x80, 0x00, 0x00, 0x40, 0x0c, 0x16,
                                                       0x25]);
//pub const DST_MAC_ADDR: MacAddress = MacAddress::Long([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
  //                                                     0xff]);


pub const DST_MAC_ADDR: MacAddress = MacAddress::Short(0xffff);
pub const MLE_DISC_REQ_PAYLOAD: [u8; 8] = [0xff, 0x10, 0x1a, 0x04, 0x80, 0x02, 0x20, 0x00];

pub const IP6_HDR_SIZE: usize = 40;
pub const PAYLOAD_LEN: usize = 16;//Must set this to UDP payload + 8
...
impl<'a, A: time::Alarm + 'a> LowpanTest<'a, A> {
    ...
    fn run_test(&self) {
        debug!("Sending a UDP packet:");
//Begin Changes for UDP testing
        ...
        //Set UDP Payload within IP6_DGRAM
        {
            ...
            for i in 0..(PAYLOAD_LEN - 8) {
                //payload[i] = 100 as u8;
                payload[i] = MLE_DISC_REQ_PAYLOAD[i]; 
            }
        }
        

        unsafe {
            let ip6_header: &mut IP6Header = mem::transmute(IP6_DGRAM.as_mut_ptr());
            let udp_header: &mut UDPHeader = mem::transmute(IP6_DGRAM[IP6_HDR_SIZE..(IP6_HDR_SIZE+8)].as_mut_ptr());
            udp_ipv6_prepare_packet(TF::Inline, 255, SAC::Inline, DAC::Inline, &mut IP6_DGRAM, IP6_DGRAM.len(), IP6_HDR_SIZE, SRC_ADDR, DST_ADDR, SRC_MAC_ADDR, DST_MAC_ADDR, MLP, 19788, 19788, ip6_header, udp_header); 
        }
        unsafe {
    ...
    unsafe fn send_ipv6_packet(&self,
        ...
        //frag_state.radio.set_pan(0xABCD);
        //tx_state.dst_pan.set(0xFFFF);
...
impl<'a, A: time::Alarm + 'a> ReceiveClient for LowpanTest<'a, A> {
    fn receive<'b>(&self, buf: &'b [u8], _len: u16, retcode: ReturnCode) {
...
//This function is not currently used by the sender. It is left here for now bc the receive code depends on it, though the receive code does not work for UDP anyway. Once the UDP receive callback has been implemented, all code below this point will be removed from this file
