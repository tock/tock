...
int dummy_cmp(char *str1, int len1, char *str2, int len2);
...
static void callback(int cb,
  ...
  {
    // FAIL
    if(dummy_cmp(data, sizeof(data), exp_ct, sizeof(exp_ct)) != 0) {
  ...
  {
    // FAIL
    if(dummy_cmp(data, sizeof(data), exp_pt, sizeof(exp_pt)) != 0) {
  ...
  int main(void)
  {
    ...
    /* INITIAL COUNTER */
    char ctr[] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0xff};
    ...
    for (int i = 0; i < 1; i++) {
      ...
      delay_ms(500);
      if(aes128_decrypt_ctr(data, ctr, sizeof(data)) < 0) {
        printf("encrypt error\r\n");
      }
    }
    return 0;
...
/* DUMMY FUNCTION TO COMPARE CHAR* ARRAYS */
int dummy_cmp(char *str1, int len1, char *str2, int len2) {
  if(len1 != len2) {
    return -1;
  }
