...
impl<'a, A: hil::time::Alarm<'a>, P: hil::pwm::PwmPin> hil::buzzer::Buzzer<'a> for PwmBuzzer<'a, A, P> {
    ...
    fn buzz(&self, command: BuzzerCommand) -> Result<(), ErrorCode> {
        match command {
            BuzzerCommand::Buzz {
                ...
                let ret = self.pwm_pin.start(frequency_hz, self.pwm_pin.get_maximum_duty_cycle() / 2);
...
impl<'a, A: hil::time::Alarm<'a>, P: hil::pwm::PwmPin> hil::time::AlarmClient for PwmBuzzer<'a, A, P> {
    fn alarm(&self) {
