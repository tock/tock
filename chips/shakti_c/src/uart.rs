...
/// Synchronous UART writer for panic output.
///
/// Implements PanicWriter by driving the SHAKTI UART with blocking output
/// (transmit_sync), so the panic message is fully flushed before the machine is
/// halted. Used only by the panic handler, not in normal kernel operation.
