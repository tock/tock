...
impl TestSha256 {
    pub fn new(sha: &'static Sha256Software<'static>,
               data: &'static mut[u8],
    ...
    pub fn run(&'static self) {
        ...
        let r =  self.sha.add_data(LeasableBuffer::new(self.data.take().unwrap()));
        if r.is_err() {
...
impl digest::ClientData<'static, 32> for TestSha256 {
    fn add_data_done(&'static self,
        ...
        match result {
            ...
            Err(e) => {debug!("Sha256Test: adding data failed: {:?}", e);},
        }
    }
}

impl digest::ClientVerify<'static, 32> for TestSha256 {
    fn verification_done(&'static self,
