...
.PHONY: ci-setup-miri
ci-setup-miri:
	@rustup component list | grep miri | grep -q installed \
...
ci-job-miri: ci-setup-miri
	...
	@for a in $$(tools/list_archs.sh); do cd arch/$$a && \
		CI=true cargo miri test && cd ../..; done
	@cd capsules && CI=true cargo miri test
	@for c in $$(tools/list_chips.sh); do cd chips/$$c && \
		CI=true cargo miri test && cd ../..; done
