    ...
    pub unsafe fn setup_system_clock(&self, clock_source: SystemClockSource) {
        ...
        match clock_source {
            ...
            SystemClockSource::RC80M => {
                ...
                (*PM_REGS)
                    .cpusel
                    .modify(CpuClockSelect::CPUDIV::SET + CpuClockSelect::CPUSEL::CLEAR);
    ...
    pub unsafe fn disable_system_clock(&self, clock_source: SystemClockSource) {
        ...
        match clock_source {
            ...
            SystemClockSource::RC80M => {
                ...
                (*PM_REGS)
                    .cpusel
                    .modify(CpuClockSelect::CPUDIV::CLEAR + CpuClockSelect::CPUSEL::CLEAR);
                while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}

                // Stop dividing peripheral clocks
                unlock(0x0000000C);
                (*PM_REGS).pbasel.modify(
                    PeripheralBusXClockSelect::PBDIV::CLEAR
                        + PeripheralBusXClockSelect::PBSEL::CLEAR,
                );
                while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}
                unlock(0x00000010);
                (*PM_REGS).pbbsel.modify(
                    PeripheralBusXClockSelect::PBDIV::CLEAR
                        + PeripheralBusXClockSelect::PBSEL::CLEAR,
                );
                while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}
                unlock(0x00000014);
                (*PM_REGS).pbcsel.modify(
                    PeripheralBusXClockSelect::PBDIV::CLEAR
                        + PeripheralBusXClockSelect::PBSEL::CLEAR,
                );
                while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}
                unlock(0x00000018);
                (*PM_REGS).pbdsel.modify(
                    PeripheralBusXClockSelect::PBDIV::CLEAR
                        + PeripheralBusXClockSelect::PBSEL::CLEAR,
                );
                while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}
...
unsafe fn configure_80mhz_rc() {
    ...
    (*PM_REGS)
        .pbasel
        .modify(PeripheralBusXClockSelect::PBDIV::SET + PeripheralBusXClockSelect::PBSEL::CLEAR);
    while (*PM_REGS).sr.matches_all(InterruptOrStatus::CKRDY::CLEAR) {}
    unlock(0x00000010);
    (*PM_REGS)
        .pbbsel
