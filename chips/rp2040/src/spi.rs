...
pub struct Spi<'a> {
    ...
    active_slave: OptionalCell<ChipSelectPin<&'a crate::gpio::RPGpioPin<'a>>>,
...
impl<'a> SpiMaster<'a> for Spi<'a> {
    ...
    fn specify_chip_select(
        &self,
        cs: Self::ChipSelect,
        cs_polarity: ChipSelectActivePolarity,
    ) -> Result<(), ErrorCode> {
        if !self.is_busy() {
            self.active_slave.set(ChipSelectPin::new(cs, cs_polarity));
            Ok(())
        } else {
            Err(ErrorCode::BUSY)
        }
