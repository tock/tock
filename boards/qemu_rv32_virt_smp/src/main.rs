...
//     #[inline(always)]
//     const fn value() -> usize { N }
//     #[inline(always)]
//     const fn new() -> Const<N> { Const{} }
// }


unsafe fn thread_local_main<const ID: usize>() {
    ...
    let board_kernel = thread_local_static_init!(
        MAX_THREADS,
        ID,
        kernel::Kernel,
    ...
    let peripherals = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    let uart_mux = components::console::UartMuxComponent::new(&peripherals.uart0, 115200)
        .finalize(components::thread_local_uart_mux_component_static!(MAX_THREADS, ID));

    // Use the RISC-V machine timer timesource
    let hardware_timer = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    // alarm.
    let mux_alarm = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    let systick_virtual_alarm = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    let virtual_alarm_user = thread_local_static_init!(
        MAX_THREADS,
        ID,
        VirtualMuxAlarm<'static, qemu_rv32_virt_chip::chip::QemuRv32VirtClint>,
        VirtualMuxAlarm::new(mux_alarm)
    ...
    let alarm = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    > = if let Some(rng_IDx) = virtio_rng_IDx {
        ...
        let descriptors = thread_local_static_init!(MAX_THREADS, ID, VirtqueueDescriptors<1>, VirtqueueDescriptors::default());
        let available_ring =
            thread_local_static_init!(MAX_THREADS, ID, VirtqueueAvailableRing<1>, VirtqueueAvailableRing::default(),);
        let used_ring = thread_local_static_init!(MAX_THREADS, ID, VirtqueueUsedRing<1>, VirtqueueUsedRing::default(),);
        let queue = thread_local_static_init!(
            MAX_THREADS,
            ID,
            SplitVirtqueue<1>,
        ...
        let rng = thread_local_static_init!(MAX_THREADS, ID, VirtIORng, VirtIORng::new(queue));
        ...
        // are routed properly
        let mmio_queues = thread_local_static_init!(MAX_THREADS, ID, [&'static dyn Virtqueue; 1], [queue; 1]);
        peripherals.virtio_mmio[rng_IDx]
            .initialize(rng, mmio_queues)
            .unwrap();

        // Provide an internal randomness buffer
        let rng_buffer = thread_local_static_init!(MAX_THREADS, ID, [u8; 64], [0; 64]);
        ...
        let rng_driver = thread_local_static_init!(
            MAX_THREADS,
            ID,
    ...
    > = if let Some(net_IDx) = virtio_net_IDx {
        ...
        // TX Virtqueue
        let tx_descriptors =
            thread_local_static_init!(MAX_THREADS, ID, VirtqueueDescriptors<2>, VirtqueueDescriptors::default(),);
        let tx_available_ring =
            thread_local_static_init!(MAX_THREADS, ID, VirtqueueAvailableRing<2>, VirtqueueAvailableRing::default(),);
        let tx_used_ring = thread_local_static_init!(MAX_THREADS, ID, VirtqueueUsedRing<2>, VirtqueueUsedRing::default(),);
        let tx_queue = thread_local_static_init!(
            MAX_THREADS,
            ID,
            SplitVirtqueue<2>,
        ...
        let rx_available_ring =
            thread_local_static_init!(MAX_THREADS, ID, VirtqueueAvailableRing<2>, VirtqueueAvailableRing::default(),);
        let rx_used_ring = thread_local_static_init!(MAX_THREADS, ID, VirtqueueUsedRing<2>, VirtqueueUsedRing::default(),);
        let rx_queue = thread_local_static_init!(
            MAX_THREADS,
            ID,
            SplitVirtqueue<2>,
        ...
        let tx_header_buf = thread_local_static_init!(MAX_THREADS, ID, [u8; 12], [0; 12]);
        let rx_header_buf = thread_local_static_init!(MAX_THREADS, ID, [u8; 12], [0; 12]);

        // Currently, provide a single receive buffer to write
        // incoming packets into
        let rx_buffer = thread_local_static_init!(MAX_THREADS, ID, [u8; 1526], [0; 1526]);

        // Instantiate the VirtIONet (NetworkCard) driver and set
        // the queues
        let virtio_net = thread_local_static_init!(
            MAX_THREADS,
            ID,
            VirtIONet<'static>,
            VirtIONet::new(
        ...
        let mmio_queues = thread_local_static_init!(MAX_THREADS, ID, [&'static dyn Virtqueue; 2], [rx_queue, tx_queue]);
    ...
    let chip = thread_local_static_init!(
        MAX_THREADS,
        ID,
    ...
    let process_printer = components::process_printer::ProcessPrinterTextComponent::new()
        .finalize(components::thread_local_process_printer_text_component_static!(MAX_THREADS, ID));
    ...
    .finalize(components::thread_local_process_console_component_static!(
        MAX_THREADS,
        ID,
        qemu_rv32_virt_chip::chip::QemuRv32VirtClint
    ));
    ...
    .finalize(components::thread_local_console_component_static!(MAX_THREADS, ID));
    // Create the debugger object that handles calls to `debug!()`.
    components::debug_writer::DebugWriterComponent::new(uart_mux)
        .finalize(components::thread_local_debug_writer_component_static!(MAX_THREADS, ID));
    ...
    .finalize(components::thread_local_low_level_debug_component_static!(MAX_THREADS, ID));

    let scheduler = components::sched::cooperative::CooperativeComponent::new(&PROCESSES)
        .finalize(components::thread_local_cooperative_component_static!(MAX_THREADS, ID, NUM_PROCS));

    let scheduler_timer = thread_local_static_init!(
        MAX_THREADS,
        ID,
