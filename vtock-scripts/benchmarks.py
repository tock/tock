    ...
    for item in output_list:
        ...
        if parts[0] == "[EVAL]":
            ...
            if fn_name not in average_hash_map:
                average_hash_map[fn_name] = (1, cycles)
            else:
                # get the current count and sum of cycles
                count, sum_cycles = average_hash_map[fn_name]
                ...
                # update the hash map
                average_hash_map[fn_name] = (count, sum_cycles)

    for key, (count, sum_cycles) in average_hash_map.items():
        average_hash_map[key] = sum_cycles / count 
