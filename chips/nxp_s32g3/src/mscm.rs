...
//! Miscellaneous System Control Module (MSCM) driver for NXP S32G3.
//!
//! Provides named IRQ number constants (RM §8.5.2.2) and an interrupt-router
//! driver (`Mscm`) that steers shared peripheral interrupts (IRSPRC registers)
//! to a target Cortex-M7 core via the MSCM interrupt router.
//!
//! All IRQ numbers are NVIC external vector indices as seen by the Cortex-M7.
...
// MSCM Interrupts
// Documented in RM §8.5.2.2 (MSCM interrupt router functional description).
//
...
impl Mscm {
    ...
    /// Routes a peripheral interrupt to the specified core via the MSCM
    /// interrupt router (IRSPRC register).
    ///
    /// # INIT-ONLY
    /// Modifies IRSPRC interrupt routing registers. Must be called before
    /// `kernel_loop()` — routing changes after the scheduler is live race with
