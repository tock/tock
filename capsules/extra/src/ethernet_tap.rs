...
//!
//! This driver can both deliver incoming IEEE 802.1 Ethernet frames from the
...
//!   local to the process: `(tx_frames, tx_bytes)`.
//!
//!   These counters are local to each process.
//!
...
impl<'a, E: EthernetAdapterDatapath<'a>> EthernetTapDriver<'a, E> {
    ...
    fn transmit_frame_from_grant(
        ...
        let transmit_buffer = match prev_state {
            // This method must never be called during an active transmission.
            TxState::Active { .. } => panic!(),
    ...
    for EthernetTapDriver<'a, E>
{
    fn transmit_frame_done(
        ...
        // Now, check for any processes that have a pending transmission. We use
        // `find` as a short-circuiting method to iterate over the grant regions
        // and only schedule a single transmission:
        self.tx_state
            .map(|tx_state| {
                self.apps.iter().for_each(|process_grant| {
                    let process_id = process_grant.processid();
                    let _ = process_grant.enter(|grant, kernel_data| {
                        ...
                        {
                            // Initialing the transmission failed. We must
                            ...
                            // Try scheduling a transmission for the next pending
                            // process:
                            false
                        } else {
                            // Transmission initiated successfully, short-circuit the
                            // iterator here:
                            true
                        }
                    });
    ...
    fn received_frame(&self, frame: &[u8], timestamp: Option<u64>) {
        ...
        frame_header[0..2].copy_from_slice(&u16::to_ne_bytes((timestamp.is_some() as u16) << 0));
