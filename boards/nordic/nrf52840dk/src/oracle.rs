    ...
    > OracleDriver<'static, A>
{
        ...
        self.apps
            .enter(processid, |app, kernel_data| {
                ...
                kernel_data
                    ...
                    .and_then(|iv|
                        iv.enter(|iv| {
                            let mut static_buf = [0; kernel::hil::symmetric_encryption::AES128_KEY_SIZE];
                ...
                self.source_buffer.map_or(Err(ErrorCode::NOMEM), |static_buf| {
                    kernel_data
                        .get_readonly_processbuffer(ro_allow::SOURCE)
                        .and_then(|source|
                            Ok(source.enter(|source| {
                                // Determine the size of the static buffer we have
                                let copy_len = core::cmp::min(static_buf.len(), source.len() - app.source_offset);
                                // Clear any previous plaintext
                                for c in static_buf.iter_mut() {
                                    *c = 0;
                                }
                                // Copy the data into the static buffer
                                source[app.source_offset..(copy_len + app.source_offset)].copy_to_slice(&mut static_buf[..copy_len]);
    ...
    fn initiate_next_segment(&self) -> Result<(), ErrorCode> {
        if let Some((e, source, dest)) = AES128::crypt(
            self.aes,
            Some(self.source_buffer.take().expect("Should never be called with empty source_buffer")),
            self.dest_buffer.take().expect("Should never be called with empty dest_buffer"),
            0,
            AES128_BLOCK_SIZE,
        ) {
...
impl<
        ...
        A: AES128<'static>
            + AES128Ctr
    > Client<'static> for OracleDriver<'static, A>
{
    fn crypt_done(&'a self, mut source: Option<&'static mut [u8]>, destination: &'static mut [u8]) {
        ...
        self.processid.map(|processid| {
            self.apps.enter(processid, |app, kernel_data| {
                let _ = kernel_data.get_readwrite_processbuffer(rw_allow::DEST).and_then(|app_dest|
                    app_dest.mut_enter(|app_dest| {
                        let remaining_copy_len = app_dest.len() - app.dest_offset;
                        let copy_len = core::cmp::min(remaining_copy_len, destination.len());
                        app_dest[app.dest_offset..(app.dest_offset + copy_len)].copy_from_slice(&destination[..copy_len]);
                        app.dest_offset += copy_len;
                    })
                );
                self.dest_buffer.replace(destination);
                let _ = kernel_data.get_readonly_processbuffer(ro_allow::SOURCE).and_then(|app_source|
                    app_source.enter(|app_source| {
                        if app.source_offset + source.len() < app_source.len() {
                            ...
                            app_source[app.source_offset..(app.source_offset + copy_len)].copy_to_slice(&mut source[..copy_len]);
                            ...
                            self.processid.clear();
                            kernel_data
                                .schedule_upcall(0, (0, app_source.len(), 0)).ok();
                        }
                    })
                );
            })
...
impl<
        ...
        A: AES128<'static>
            + AES128Ctr
    > SyscallDriver for OracleDriver<'static, A>
{
    fn command(
        &self,
        command_num: usize,
        ...
        let ret = self.apps.enter(processid, |app, _kernel_data| {
            match command_num {
                // check if present
