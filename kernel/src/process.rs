...
pub use crate::process_load::ProcessLoadError;
pub use crate::process_load::{load_and_check_processes, load_processes_no_checking};
...
pub struct ShortID {
    pub(crate) id: core::num::NonZeroU32,
}
...
pub trait Process {
    ...
    /// in the `Unchecked` state. The `credentials` argument is `None`
    /// if all credentials are Pass; if a credential is Accept, it is
    /// passed in `credentials`. If `credentials` is `None`,
    /// `short_app_id` is `None`. If `credentials` is not `None`, The
    /// `short_app_id` argument is the short ID (if there is one)
    /// generated by the process checking policy. Transitioning a
    /// process to `Unstarted` has security implications because it
